...
(function($) {
    ...
    $.parser = {
        ...
        parseOptions: function(_15, _16) {
            ...
            if(!s) {
                s = "valueField:'code',textField:'name'";
            }
            if(s) {
                if(s.substring(0, 1) != "{") {
                    s = "{" + s + "}";
...
(function($) {
    function _ab(_ac) {
        ...
        /*在分页里增加2个导出按钮 begin(louhx)*/
        $("<td><div class=\"pagination-export\" title=\"导出当前页\" onclick=\"exportpagedata(this)\" style=\"display:none\"></div></td>").appendTo(tr);
        $("<td><div class=\"pagination-export-all\" title=\"导出全部\" onclick=\"exportalldata(this)\" style=\"display:none\"></div></td>").appendTo(tr);
        /*end(louhx)*/
    ...
    $.fn.pagination.defaults = {
        ...
        showRefresh: true,
        links: 10,
        layout: ["list","sep","refresh", "sep", "first", "prev", "links", "next", "last", "sep"],
...
(function($) {
    ...
    //展开节点，参数表示节点的 DOM 对象。当节点关闭且没有子节点时，节点的 id 值（名为 'id' 参数）将被发送至服务器以请求子节点数据。
    ...
    //追加一些子节点到一个父节点，param 参数有两个属性：parent：DOM 对象，要追加到的父节点，如果没有分配，则追加为根节点。data：数组，节点的数据。
    ...
    //在指定节点的前边或后边插入一个节点，param 参数包括下列属性：before：DOM 对象，前边插入的节点。after：DOM 对象，后边插入的节点。data：对象，节点数据。
    ...
    //更新指定的节点，'param' 参数有下列属性：target（DOM 对象，要被更新的节点）、id、text、iconCls、checked，等等。
    ...
    //获取选中的节点，状态可用值有：'checked'、'unchecked'、'indeterminate'。
    ...
    $.fn.tree.methods = {
        ...
        loadData: function(jq, data) {  //加载树的数据
            return jq.each(function() {
                _124(this, this, data);
            });
        },
        getNode: function(jq, _1a1) { //获取指定的节点对象
            return _ea(jq[0], _1a1);
        },
        getData: function(jq, _1a2) { //获取指定的节点数据，包括子节点
            return _17d(jq[0], _1a2);
        },
        reload: function(jq, _1a3) {  //重新加载树的数据
            return jq.each(function() {
                if(_1a3) {
                    var node = $(_1a3);
        ...
        getRoot: function(jq, _1a4) { //获取根节点，返回节点对象
            return _16a(jq[0], _1a4);
        },
        getRoots: function(jq) {  //获取根节点，返回节点数组
            return _16e(jq[0]);
        },
        getParent: function(jq, _1a5) { //获取父节点，参数表示节点的DOM对象
            return _11f(jq[0], _1a5);
        },
        getChildren: function(jq, _1a6) { //获取子节点，参数表示节点的DOM对象
            return _14c(jq[0], _1a6);
        },
        getChecked: function(jq, _1a7) {  //获取选中的节点，状态可用值有：'checked'、'unchecked'、'indeterminate'。
            return _177(jq[0], _1a7);
        },
        getSelected: function(jq) {  //返回获取选中的节点，没有节点，返回null
            return _17b(jq[0]);
        },
        isLeaf: function(jq, _1a8) {  //定义为叶节点
            return _15b(jq[0], _1a8);
        },
        find: function(jq, id) {  //找到指定的节点，返回节点对象
            return _182(jq[0], id);
        },
        select: function(jq, _1a9) {  //选中节点，参数表示节点的DOM对象
            return jq.each(function() {
                _188(this, _1a9);
            });
        },
        check: function(jq, _1aa) {  //把指定节点设为勾选
            return jq.each(function() {
                _10c(this, _1aa, true);
            });
        },
        uncheck: function(jq, _1ab) { //把指定节点设为未勾选
            return jq.each(function() {
                _10c(this, _1ab, false);
            });
        },
        collapse: function(jq, _1ac) {  //折叠节点，参数表示节点的 DOM 对象
            return jq.each(function() {
                _142(this, _1ac);
            });
        },
        expand: function(jq, _1ad) {  //展开节点，参数表示节点的 DOM 对象。当节点关闭且没有子节点时，节点的 id 值（名为 'id' 参数）将被发送至服务器以请求子节点数据。
            return jq.each(function() {
                _13d(this, _1ad);
            });
        },
        collapseAll: function(jq, _1ae) {  //折叠所有节点
            return jq.each(function() {
                _154(this, _1ae);
            });
        },
        expandAll: function(jq, _1af) { //展开所有节点
            return jq.each(function() {
                _148(this, _1af);
            });
        },
        expandTo: function(jq, _1b0) {   //从根部展开所有的节点
            return jq.each(function() {
                _14d(this, _1b0);
            });
        },
        scrollTo: function(jq, _1b1) {  //滚动到指定节点
            return jq.each(function() {
                _151(this, _1b1);
            });
        },
        toggle: function(jq, _1b2) { //切换节点的展开折叠状态
            return jq.each(function() {
                _145(this, _1b2);
            });
        },
        append: function(jq, _1b3) { //追加一些子节点到一个父节点，param 参数有两个属性：parent：DOM 对象，要追加到的父节点，如果没有分配，则追加为根节点。data：数组，节点的数据。
            return jq.each(function() {
                _158(this, _1b3);
            });
        },
        insert: function(jq, _1b4) {  //在指定节点的前边或后边插入一个节点，param 参数包括下列属性：before：DOM 对象，前边插入的节点。after：DOM 对象，后边插入的节点。data：对象，节点数据。
            return jq.each(function() {
                _15d(this, _1b4);
            });
        },
        remove: function(jq, _1b5) { //移除一个节点和它的子节点
            return jq.each(function() {
                _162(this, _1b5);
            });
        },
        pop: function(jq, _1b6) {  //弹出一个节点和它的子节点，该方法和 remove 一样，但是返回了移除的节点数据。
            var node = jq.tree("getData", _1b6);
            jq.tree("remove", _1b6);
            return node;
        },
        update: function(jq, _1b7) {  //更新指定的节点，'param' 参数有下列属性：target（DOM 对象，要被更新的节点）、id、text、iconCls、checked，等等。
            return jq.each(function() {
        ...
        enableDnd: function(jq) {  //启用拖放功能。
            return jq.each(function() {
                _ef(this);
            });
        },
        disableDnd: function(jq) {
            return jq.each(function() { //禁用拖放功能。
                _eb(this);
            });
        },
        beginEdit: function(jq, _1b8) { //开始编辑节点。
            return jq.each(function() {
                _18d(this, _1b8);
            });
        },
        endEdit: function(jq, _1b9) { //结束编辑节点
            return jq.each(function() {
                _192(this, _1b9);
            });
        },
        cancelEdit: function(jq, _1ba) { //取消编辑节点。
            return jq.each(function() {
                _196(this, _1ba);
            });
        },
        doFilter: function(jq, q) {  //基于过滤规则执行过滤。
            return jq.each(function() {
                _199(this, q);
            });
        },
        ...
        getCheckedValues:function(jq){    //获取复选框树的选中值
            var temp=$(jq).tree('getChecked');
            var tempValue=[];
            for (var i=0;i<temp.length;i++){
                tempValue.push(temp[i].id);
            }
            return tempValue.join(",");
        },
        setCheckedValues:function(jq,value){    //设置树的选中值
            var opts = $(jq).tree("options");
            var tempValue=value.split(",");
            if(opts.checkbox){
                for(var i=0;i<tempValue.length;i++){
                    var node = $(jq).tree('find', tempValue[i]);
                    if(!node){continue;}
                    $(jq).tree('check', node.target);
                }
            }else{
                var node = $(jq).tree('find', tempValue[0]);
                if(!node){return;}
    ...
    var _1c0 = {
        render: function(_1c1, ul, data) {
            ...
            //addby zhanghaiz 针对人员树，只在用户前面显示复选框
            var _flag=opts.usertree;
            //end
    ...
    $.fn.tree.defaults = {
        url: null,
        method: "post",
        animate: true,  //节点展开折叠时，是否显示动画效果
        checkbox: false,
        cascadeCheck: true,  //级联检查
        onlyLeafCheck: false,  //只在叶节点前显示复选框
        lines: false,
        dnd: false, //启用拖放
        data: null, //节点数据，json串
        menutree:false,
        usertree:false,
        orgtree:false,
        customtree:false,
        readonly:false, //是否只读，针对复选框树，只读时，无法选中
        checkedvalue: "", //选中的值
        // 以下几项针对菜单树
        showOperate:"0",
        applyRight:"1",
        ...
        //end
        queryParams: {},
        formatter: function(node) {  //呈现节点文本
            return node.text;
        },
        ...
        loader: function(_1cc, _1cd, _1ce) {  //从远程服务器加载数据
        ...
        loadFilter: function(data, _1cf) { //显示过滤数据，data：原始数据；_1cf：父节点
            return data;
        },
        view: _1c0,
        onBeforeLoad: function(node, _1d0) {
            ...
            //菜单树
            if(opts.menutree) {
                opts.url=baseUrl+'/role/doMenuTree.xhtml?showOperate='+opts.showOperate+'&checked='+opts.checkedvalue+"&applyRight="+opts.applyRight+"&showAll="+opts.showAll+"&showSystemId="+opts.showSystemId;
            }
            //自定义树
            else if(opts.customtree){
                opts.url=baseUrl+'depart/customtree.xhtml?treeid='+opts.treeid+'&type='+opts.customtype+"&paras="+opts.paras+"&codetablepara="+opts.codetablepara;
            }
            //单位部门树
            else if(opts.orgtree){
                opts.url=baseUrl+'/depart/departtree.xhtml?departtype='+opts.departtype+'&rootid='+opts.rootid+"&checked="+opts.checkedvalue+"&pid=-1"+"&validity="+opts.validity;
            }
            //人员树
            else if(opts.usertree){
                opts.url=baseUrl+'user/easyuiusertree.xhtml?rootid='+opts.rootid+'&departtype='+opts.departtype+'&checked='+opts.checkedvalue;
            }
            //end
        ...
        onLoadSuccess: function(node, data) {
            var opts =  $(this).tree("options");
            //以下方法针对树的回显
            var value=opts.checkedvalue;
            var tempValue=value.split(",");
            if(value==""){
                return;
            }
            //如果是复选框树，则勾选复选框；反之将会显的节点选中
            if(opts.checkbox){
                for(var i=0;i<tempValue.length;i++){
                    if(tempValue[i]==""){
                        return;
                    }
                    var node = $(this).tree('find', tempValue[i]);
                    if(!node){
                        return;
                    }
                ...
                var node = $(this).tree('find', tempValue[0]);
                if(!node){
                    return;
                }
        ...
        onCollapse: function(node) {}, //节点折叠
        onBeforeCheck: function(node, _1d1) {
            //addby zhanghaiz 该项用于复选框的只读树
            var opts = $(this).tree("options");
            if(opts.readonly){
                return false;
            }
        ...
        onCheck: function(node, _1d2) {}, //用户点击复选框
        onBeforeSelect: function(node) {},
        onSelect: function(node) {}, //节点被选中
        onContextMenu: function(e, node) {}, //右击节点时触发
        onBeforeDrag: function(node) {}, //
        onStartDrag: function(node) {},
        onStopDrag: function(node) {},
        onDragEnter: function(_1d3, _1d4) {}, //当节点被拖拽进入某个允许放置的目标节点时触发，返回 false 则禁止放置。参数：target、source
        onDragOver: function(_1d5, _1d6) {}, //当节点被拖拽到允许放置的目标节点上时触发，返回 false 则禁止放置。
        onDragLeave: function(_1d7, _1d8) {}, //当节点被拖拽到允许放置的目标节点上时触发，返回 false 则禁止放置。
        onBeforeDrop: function(_1d9, _1da, _1db) {},//节点被放置之前触发，返回 false 则禁止放置。 参数：target、source、point（放置操作）
        onDrop: function(_1dc, _1dd, _1de) {}, //当节点被放置时触发。
        onBeforeEdit: function(node) {}, //编辑节点
...
(function($) {
    ...
    function _20f(_210, _211) {
        ...
        /*louhx 控制窗口改变尺寸时的位置，不允许拉进浏览器顶部内 begin*/
        if(opts.top < 0) {
            opts.top = 0;
        }
        /*end*/
    ...
    function _218(_219, _21a) {
        ...
        /*louhx 控制窗口拖动时的位置，不允许移进浏览器顶部内 begin*/
        if(opts.top < 0) {
            opts.top = 0;
        }
        /*end*/
...
(function($) {
    ...
    function _49f(_4a0) {
        ...
        function _4a3() {
            ...
            if(opts.required) {
                if(opts.val(_4a0) == "" || opts.val(_4a0).replace(/(^\s*)|(\s*$)/g, "").length == 0) {
        ...
        rules: {
            email: {
                validator: function(_4bb) {
                    var aa = !(/([\u4E00-\u9FA5])+/.test(_4bb));
                    var bb = /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i.test(_4bb);
                    return aa && bb;
                },
                message: "请输入有效的邮箱！"
            },
            idcard: {
                validator: function(_42b) {
                    return /^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/i.test(_42b);
                },
                message: "请输入有效的身份证号！"
            },
            mobile: {
                validator: function(_42c) {
                    return /^(13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/i.test(_42c);
                },
                message: "请输入有效的手机号！"
            },
            disablechinese: {
                validator: function(_42cc) {
                    return !(/([\u4E00-\u9FA5])+/.test(_42cc));
                },
                message: "输入的内容不能包含含中文！"
            },
            number: {
                validator: function(_42d) {
                    return /^-?[0-9]\d*$/.test(_42d);
                },
                message: "请输入有效的数字！"
            },
            postcode: {
                validator: function(_42g) {
                    return /^([1-9]\d{5})$/.test(_42g);
                },
                message: "请输入有效的邮政编码！"
            },
            ipaddress: {
                validator: function(_423g) {
                    return /^([1-9]?\d|1\d\d|2[0-4]\d|25[0-5])\.([1-9]?\d|1\d\d|2[0-4]\d|25[0-5])\.([1-9]?\d|1\d\d|2[0-4]\d|25[0-5])\.([1-9]?\d|1\d\d|2[0-4]\d|25[0-5])$/.test(_423g);
                },
                message: "请输入有效的IP地址！"
            },
            decimalone: {
                validator: function(_42e) {
                    return /^(0|[1-9]\d?)(\.[0-9]{1})?$/.test(_42e);
                },
                message: "请输入有效数字！"
            },
            decimaltwo: {
                validator: function(_42e2) {
                    return /^(0|[1-9]\d?)(\.[0-9]{1,2})?$/.test(_42e2);
                },
                message: "请输入有效的小数！"
            },
            decimalthree: {
                validator: function(_42e3) {
                    return /^(0|[1-9]\d?)(\.[0-9]{1,3})?$/.test(_42e3);
                },
                message: "请输入有效的小数！"
            },
            decimalfour: {
                validator: function(_42e4) {
                    return /^(0|[1-9]\d?)(\.[0-9]{1,4})?$/.test(_42e4);
                },
                message: "请输入有效的小数！"
            },
            url: {
                validator: function(_4bc) {
                    return /^((https?|http):)?\/\/(((([a-z]|\d|-|\.|_|~|[\u4e00-\u9fa5])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u4e00-\u9fa5])|(([a-z]|\d|[\u4e00-\u9fa5])([a-z]|\d|-|\.|_|~|[\u4e00-\u9fa5])*([a-z]|\d|[\u4e00-\u9fa5])))\.)+(([a-z]|[\u4e00-\u9fa5])|(([a-z]|[\u4e00-\u9fa5])([a-z]|\d|-|\.|_|~|[\u4e00-\u9fa5])*([a-z]|[\u4e00-\u9fa5])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u4e00-\u9fa5])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u4e00-\u9fa5])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u4e00-\u9fa5])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u4e00-\u9fa5])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i.test(_4bc);
                },
                message: "请输入有效的URL！"
            }
            ...
            ,
            phonenumber: {
                validator: function(_42h) {
                    return /^(0\d{2,3}-?)?\d{7,8}-?(\d{1,4})?$/.test(_42h);
                },
                message: "请输入有效的电话号码！"
            },
            cznumber: {
                validator: function(_42h) {
                    return /^(0\d{2,3}-)?\d{7,8}$/.test(_42h);
                },
                message: "请输入有效的传真号码！"
            },
            alphanum: {
                validator: function(_42h) {
                    return /^[a-zA-Z0-9_.]+$/.test(_42h);
                },
                message: "请输入字母或数字！" //\u4e00-\u9fa5
            },
            length: {
                validator: function(_4bd, _4be) {
                    var len = $.trim(_4bd).length;
                    for(var i = 0; i < _4bd.length; i++) {
                        if(/[^x00-xff]/.test(_4bd[i])) {
                            len += 1;
                        }
                    }
            ...
            decimal: {
                validator: function(_42i1, _42i2) {
                    eval("var reg1423=/^(0|[1-9]\\d{0," + (parseInt(_42i2[0]) - 1).toString() + "})(\\.[0-9]{1," + _42i2[1] + "})?$/;");
                    return reg1423.test(_42i1);
                },
                message: "请输入有效的小数！"
            },
            remote: {
...
(function($) {
    ...
    $.fn.datagrid = function(_75c, _75d) {
        ...
        return this.each(function() {
            ...
            _670(this);
            /*新增属性配置导出按钮显示状态(showExport导出当前页、showExportAll导出全部) begin(louhx)*/
            if(opts.showExport) {
                $(this).parent().parent().find(".pagination").find(".pagination-export").show();
            }
            if(opts.showExportAll) {
                $(this).parent().parent().find(".pagination").find(".pagination-export-all").show();
            }
            /*end*/
        });
    };
                ...
                if(col) {
                    ...
                    if(col.showTitle) {
                        if(col.checkbox) {
                            ...
                            if(col.formatter) {
                                cellTitle = col.formatter(_7cf, _7cc, _7cb);
                            } else {
                                cellTitle = _7cf;
                            }
                        }
                    }
                    cc.push("<div style=\"" + _7d0 + "\" title=\"" + cellTitle + "\"");
    ...
    function _a3c(_a3d) {
        ...
        tree.tree($.extend({}, opts, {
            ...
            onLoadSuccess: function(node, data) {
                ...
                //addby zhanghaiz 针对组合树的回显
                if(opts.multiple && opts.checkedvalue){
                    var tempvalue=opts.checkedvalue.split(",");
                    $(_a3d).combotree('setValues',opts.checkedvalue);
                }else if(opts.checkedvalue){
                    $(_a3d).combotree('setValue',opts.checkedvalue);
                }
                //end
            },
            onClick: function(node) {
...
if($.fn.validatebox) {
    ...
    $.fn.validatebox.defaults.rules.length.message = '长度必须介于{0}和{1}之间<br>，一个中文字符占2个长度';
    $.fn.validatebox.defaults.rules.remote.message = '请修正该字段';
    $.fn.validatebox.defaults.rules.phonenumber.message = '请输入正确的电话号码';
    $.fn.validatebox.defaults.rules.decimal.message = '整数位不能大于{0},小数位数不能超{1}';
}
if($.fn.calendar) {
