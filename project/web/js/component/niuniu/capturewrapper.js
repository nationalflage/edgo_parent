...
function captureInit()
{
    ...
    captureObj.NiuniuAuthKey = $.md5("niuniu_huayu_web_"+getNowFormatDate());
    //此处可以设置相关参数
...
{
    $('#btnUpload').hide();
    var vchr_domain= "wzjc";
    var vchr_entityid= "class";
    //上传的数据除了图片外，还可以包含自己需要传递的参数
    var data ="picdata=" + encodeURIComponent(savedPictureContent)+"&vchr_entityid="+vchr_entityid+"&vchr_domain="+vchr_domain+"&fileType=1&Random="+Math.random();

    $.ajax({
        type: "POST",
        url: baseUrl+ "/upload/doUploadCapture.xhtml",
        dataType: "json",
        data: data,
        success: function (obj) {
            if(obj.code==0)
            {
                $.hyjx.alert("提示信息","上传成功！","info", function(){
                    $("#btnUpload").imgupload("show",{
                        vchr_entityid:vchr_entityid,
                        vchr_domain:vchr_domain
                    });
                });
                parent.$('.right-menu').find('li').addClass('active');
                parent.$('.right-menu-layer').show();
            }
            else
            {
                $.hyjx.alert("提示","修改失败，请重试！");
            }
        },
        error : function(){$.hyjx.alert('由于网络原因，上传失败。'); }
    });
}
