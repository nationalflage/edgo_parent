...
(function ($) {
    if($.fn.ajaxForm == undefined) {
    	document.write("<script language=javascript src=\""+baseUrl+"js/jquery-upload-file/jquery.form.js\"></script>");
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            dragDropStr: "<span><b>拖放文件至此</b></span>",
            uploadStr:"上传",
            abortStr: "终止",
            cancelStr: "取消",
            deletelStr: "删除",
            doneStr: "完成",
            multiDragErrorStr: "不支持拖拽多个文件",
            extErrorStr: "不支持的扩展名. 支持下列扩展名: ",
            duplicateErrorStr: "is not allowed. File already exists.",
            sizeErrorStr: "文件过大: ",
            uploadErrorStr: "Upload is not allowed",
            maxFileCountErrorStr: "超过同时上传数量限制，最多同时上传:",
            downloadStr: "下载",
        ...
        this.errorLog = $("<div id='errorLog"+options.unicode+"' ></div>"); //Writing errors
        this.responses = [];
	   ...
	   this.startUpload = function () {
        ...
        function createCustomInputFile (obj, group, s, uploadLabel) {
            ...
            var form = $("<form id='form"+s.unicode+"'  method='" + s.method + "' action='" + s.url + "' enctype='" + s.enctype + "'></form>");
		...
		/*	var identify = document.cookie.split("=")[1];
			//alert(identify);
			if($(identify).data("alreadyFileCount")>=$(identify).data("maxFileCount")){
            		$("<div class='" + s.errorClass + "'><b>" + "您上传的图片已经超过最大数量" +$(identify).data("maxFileCount")+"</b> " +  "</div>").appendTo(
                            obj.errorLog);
                        return false; 
        	}else{
            ...
            this.view = $("<div>查看</div>").appendTo(this.statusbar).hide();
            ...
            this.view.addClass("ajax-file-upload-green");
            ...
            var options = {
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.returnType == "json" && $.type(data) == "object" && data.hasOwnProperty(s.customErrorKeyStr)) {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span class='" + s.errorClass + "'>错误: " + msg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
                    ...
                    if(s.showStatusAfterSuccess) {
                        ...
                        {
                        	pd.view.show();
                        	pd.view.click(function(){
                        		if(s.viewCallback)
                        			s.viewCallback.call(this, data, pd);
                        	});
                        }
                    } else {
                        pd.statusbar.hide("slow");
                ...
                error: function (xhr, status, errMsg) {
                    ...
                    if(xhr.statusText == "abort") //we aborted it
                    {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span class='" + s.errorClass + "'>错误: " + errMsg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
