...
<script type="text/javascript">
    ...
    function doQuery(){
        ...
        $('#tt').datagrid({
            url: '<%=basePath %>hanr/queryList.xhtml',
    ...
    function add()
    {
        openWinW('<%=basePath %>hanr/gotoAdd.xhtml',"信息添加","800px","450px",false,false);
    }
    function edit(id)
    {
        openWinW('<%=basePath %>hanr/gotoEdit.xhtml?id='+id,"信息修改","800px","450px",false,false);
    }
    function del(id)
    {
        $.messager.confirm("提示信息","确定要删除选择的数据吗？",function(result){
            if(result == true){
                $.ajax({
                    url: "<%=basePath %>hanr/gotoDelete.xhtml?ids="+id,
    ...
    function view(id)
    {
        openWinW('<%=basePath %>hanr/gotoView.xhtml?id='+id,"信息详细","650px","350px",false,false);
    }
</script>
