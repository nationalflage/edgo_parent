...
<script type="text/javascript">
    ...
    function doQuery() {
        ...
        $('#tt').datagrid({
            url: '<%=basePath %>qiaoyc/queryList.xhtml',
    ...
    function add() {
        openWinW('<%=basePath %>qiaoyc/gotoAdd.xhtml', "信息添加", "650px", "520px", false, false);
    }
    function edit(id) {
        openWinW('<%=basePath %>qiaoyc/gotoEdit.xhtml?id=' + id, "信息修改", "650px", "520px", false, false);
    }
    ...
    function del(id) {
        $.messager.confirm("提示信息", "确定要删除选择的数据吗？", function (result) {
            if (result == true) {
                $.ajax({
                    url: "<%=basePath %>qiaoyc/gotoDelete.xhtml?ids=" + id,
    ...
    function view(id) {
        openWinW('<%=basePath %>qiaoyc/gotoView.xhtml?id=' + id, "信息详细", "650px", "520px", false, false);
    }
</script>
