...
<form id="form1" class="query-list" method="post" action="" data-options="novalidate:true">
    <table class="table-query">
        ...
        <tr>
            ...
            <th>爱好</th>
            <td>
                <input type="text" name="ah" id="ah" value="${model.ah }"
                       class="easyui-textbox"/>
            </td>
            <th>性别</th>
            <td>
...
<script type="text/javascript">
    ...
    function doQuery(){
        ...
        $('#tt').datagrid({
            url: '<%=basePath %>pujl/PujlGetList.xhtml',
            /* 在请求远程数据的时候发送额外的参数*/
    ...
    function add()
    {
        openWinW('<%=basePath %>pujl/PujlGotoAdd.xhtml',"信息添加","900px","500px",false,false);
    }
    function edit(id)
    {
        openWinW('<%=basePath %>pujl/PujlGotoEdit.xhtml?id='+id,"信息修改","900px","500px",false,false);
    }
    function del(id)
    {
        $.messager.confirm("提示信息","确定要删除选择的数据吗？",function(result){
            if(result == true){
                $.ajax({
                    url: "<%=basePath %>pujl/PujlDelete.xhtml?ids="+id,
    ...
    function view(id)
    {
        openWinW('<%=basePath %>pujl/PujlGotoView.xhtml?id='+id,"信息详细","900px","500px",false,false);
    }
</script>
