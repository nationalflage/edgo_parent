...
<form id="form1" class="query-list" method="post" action="">
		<table  class="table-query">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="birthday" id="birthday" value="${model.birthday }"
                           class="easyui-datebox" required="true" data-options="validType:length[0,50]"/>
                </td>
            </tr>
            <tr>
                <th>出生地省</th>
                <td>
                    <select type="text" name="csdsheng" id="province" value="${model.csdsheng }" text="${model.csdsheng}" codetable="district"
                            class="easyui-combobox hy-codeselect"  data-options="validType:length[0,50]"/>
                </td>
                <th>出生市</th>
                <td>
                    <select  class="easyui-combobox hy-codeselect" name="csdshi" codetable="district"  parentcode="${model.csdsheng}" text="${model.csdshi}"
                             parentobject="province" id="city" style="width: 180px;"  ></select>
                </td>
            </tr>
            <tr>
                <th>出生县</th>
                <td>
                    <select class="easyui-combobox hy-codeselect" name="csdxian" parentcode="${model.csdshi}" text="${model.csdxian}"
                            codetable="district" parentobject="city" id="area" style="width: 180px;" ></select>
                </td>
                <th>具体门牌号</th>
                <td>
            ...
            <tr>
                <th>部门名称</th>
                <td>
                    <select class="easyui-combobox hy-codeselect" text="${model.bmmc}" codetable="lyw_bm" name="bmmc" id="bmmc" style="width: 180px;"></select>
                </td>
                <th>是否有效</th>
                <td>
            ...
            <tr>
                ...
                <td>
                    <c:forEach items="${ahList}" var="lyw">
                        <input name="lywAh" type="checkbox"  id="ah${lyw.code}" value="<c:out value='${lyw.code}'/>"/>
                ...
                <td>
                    <c:forEach items="${lyw_ly}" var="lyw">
                        <input name="lywLy" type="checkbox"  id="ly${lyw.code}" value="<c:out value='${lyw.code}'/>"/>
                        <c:out value='${lyw.name}'/>
                    </c:forEach>
                </td>
            </tr>
            <tr>
                <th>最擅长领域</th>
                <td>
                    <select class="easyui-combobox hy-codeselect" text="${model.zscly}" codetable="lyw_ly" name="zscly" id="zscly" style="width: 180px;"></select>
                </td>
                <th>创建人姓名</th>
                <td>
...
<script type="text/javascript">
	$(function(){
        ...
        var ah = '${model.lywAh}'.split(',');
        for(var i=0;i<ah.length;i++) {
            $("#ah"+$.trim(ah[i])).attr("checked","checked");
        }
        var ly = '${model.lywLy}'.split(',');
	...
	function save(){
		...
		$.ajax({
			type: "POST",
		   	url: "<%=basePath %>lywbuild_/doEdit.xhtml",
		   	data: data,
