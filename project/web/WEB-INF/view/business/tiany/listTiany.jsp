...
<script type="text/javascript">
		...
		function doQuery(){
			...
			$('#tt').datagrid({
				url: '<%=basePath %>tiany/queryList.xhtml',//加载的URL
				...
				columns:[[
						...
						{field:'csdxian',title:'出生地县',width:100},
		...
		function add()
	 	{
	 		openWinW('<%=basePath %>tiany/gotoAdd.xhtml',"信息添加","800px","500px",false,false);
	 	}
	 	function edit(id)
	 	{
	 		 openWinW('<%=basePath %>tiany/gotoEdit.xhtml?id='+id,"修改","800px","500px",false,false);
	 	}
	 	function del(id)
	 	{
	 		$.messager.confirm("提示信息","确定要删除选择的数据吗？",function(result){
    			if(result == true){
    				$.ajax({
    				   	url: "<%=basePath %>tiany/gotoDelete.xhtml?id="+id,
	 	...
	 	function view(id)
	 	{
	 		openWinW('<%=basePath %>tiany/gotoView.xhtml?id='+id,"信息详细","800px","500px",false,false);
	 	}
	</script>
