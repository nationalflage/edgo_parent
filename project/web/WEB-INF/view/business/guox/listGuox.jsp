...
<script type="text/javascript">
		...
		function doQuery(){
			...
			$('#tt').datagrid({
				url: '<%=basePath %>guox/queryList.xhtml',
		...
		function add()
	 	{
	 		openWinW('<%=basePath %>guox/gotoAdd.xhtml',"信息添加","1024px","480px",false,false);
	 	}
	 	function edit(id)
	 	{
	 		 openWinW('<%=basePath %>guox/gotoEdit.xhtml?id='+id,"信息修改","1024px","480px",false,false);
	 	}
	 	function del(id)
	 	{
	 		$.messager.confirm("提示信息","确定要删除选择的数据吗？",function(result){
    			if(result == true){
    				$.ajax({
    				   	url: "<%=basePath %>guox/gotoDelete.xhtml?ids="+id,
	 	...
	 	function view(id)
	 	{
	 		openWinW('<%=basePath %>guox/gotoView.xhtml?id='+id,"信息详细","650px","350px",false,false);
	 	}
	</script>
