...
<script type="text/javascript">
    ...
    function doQuery(){
        ...
        $('#tt').datagrid({
            url: '<%=basePath %>ouyq/queryList.xhtml',
            /* 在请求远程数据的时候发送额外的参数*/
    ...
    function add()
    {
        openWinW('<%=basePath %>ouyq/gotoAdd.xhtml',"信息添加","900px","500px",false,false);
    }
    function edit(id)
    {
        openWinW('<%=basePath %>ouyq/gotoEdit.xhtml?id='+id,"信息修改","900px","500px",false,false);
    }
    function del(id)
    {
        $.messager.confirm("提示信息","确定要删除选择的数据吗？",function(result){
            if(result == true){
                $.ajax({
                    url: "<%=basePath %>ouyq/gotoDelete.xhtml?ids="+id,
    ...
    function view(id)
    {
        openWinW('<%=basePath %>ouyq/gotoView.xhtml?id='+id,"信息详细","900px","500px",false,false);
    }
</script>
