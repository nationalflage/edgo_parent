...
<script type="text/javascript">
		...
		function doQuery(){
			...
			$('#tt').datagrid({
				url: '<%=basePath %>zhangy/queryList.xhtml',
		...
		function add()
	 	{
	 		openWinW('<%=basePath %>zhangy/gotoAdd.xhtml',"信息添加","650px","350px",false,false);
	 	}
	 	function edit(id)
	 	{
	 		 openWinW('<%=basePath %>zhangy/gotoEdit.xhtml?id='+id,"信息修改","650px","350px",false,false);
	 	}
	 	function del(id)
	 	{
	 		$.messager.confirm("提示信息","确定要删除选择的数据吗？",function(result){
    			if(result == true){
    				$.ajax({
    				   	url: "<%=basePath %>zhangy/gotoDelete.xhtml?ids="+id,
	 	...
	 	function view(id)
	 	{
	 		openWinW('<%=basePath %>zhangy/gotoView.xhtml?id='+id,"信息详细","650px","350px",false,false);
	 	}
	</script>
