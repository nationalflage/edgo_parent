...
<script type="text/javascript">
		...
		function doQuery(){
			...
			$('#tt').datagrid({
				url: '<%=basePath %>qiaoyunchun/queryList.xhtml',
				...
				columns:[[
						...
						{field:'zsc',title:'最擅长领域',width:100},
		...
		function add()
	 	{
	 		openWinW('<%=basePath %>qiaoyunchun/gotoAdd.xhtml',"信息添加","650px","520px",false,false);
	 	}
	 	function edit(id)
	 	{
	 		 openWinW('<%=basePath %>qiaoyunchun/gotoEdit.xhtml?id='+id,"信息修改","650px","520px",false,false);
	 	}
	 	function del(id)
	 	{
	 		$.messager.confirm("提示信息","确定要删除选择的数据吗？",function(result){
    			if(result == true){
    				$.ajax({
    				   	url: "<%=basePath %>qiaoyunchun/gotoDelete.xhtml?ids="+id,
	 	...
	 	function view(id)
	 	{
	 		openWinW('<%=basePath %>qiaoyunchun/gotoView.xhtml?id='+id,"信息详细","650px","520px",false,false);
	 	}
	</script>
