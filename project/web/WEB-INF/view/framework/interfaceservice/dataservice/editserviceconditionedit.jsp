...
<body>
	<form id="form1" class="query-list" method="post" action="">
		<table class="table-query">
			<tr>
				<th>参数名称</th>
				<td><input value="<c:out value="${model.parameter }"/>" type="text" id="parameter" name="parameter" data-options="required:true"
					size="40" class="easyui-textbox" validType="length[0,32]" /></td>
				<th>参数类型</th>
				<td width="30%">
				<input type="radio" name="type" id="type1" <c:if test="${model.type  eq '1'}">checked="checked"</c:if> value="1"/> 字符串
				<input type="radio" name="type" id="type0" <c:if test="${model.type  eq '0'}">checked="checked"</c:if> value="0"/> 整型
			...
			<tr>
				<th>字段内容</th>
				<td width="30%"><input value="<c:out value="${model.field_content }"/>" id="field_content" name="field_content" data-options="required:true"
				...
				<th>参数是否必须</th>
				<td width="30%">
				<input type="radio" name="required" id="yes" <c:if test="${model.required  eq '1'}">checked="checked"</c:if> value="1"/> 是
				<input type="radio" name="required" id="no" <c:if test="${model.required  eq '0'}">checked="checked"</c:if> value="0"/> 否
				</td>
			...
			<tr>
				...
				<td colspan="3"><input value="<c:out value="${model.parameter_means }"/>"
			...
			<tfoot>
				<tr height="100">
					<td colspan="4"><input type="button" class="btn btn-info"
						...
						value="关　闭" onclick="window.parent.closeWin('参数信息修改')" /></td>
				</tr>
			</tfoot>
		</table>
...
<script type="text/javascript">
	...
	function save(){
		...
		$.ajax({
			type: "POST",
		   	url: "<%=basePath %>dataservice/doEditConditionEdit.xhtml",
		   	data:data,
