    ...
    <script type="text/javascript" language="javascript">
    	function fn_load(){
    		<c:if test="${dataMap['type'] == 'add'}">
            	document.all.OA1.Open("<%=webapp%>/webword/webwordAction!makeword.dhtml?dataMap['BUSPKEY']=${dataMap['BUSPKEY']}&dataMap['TEPID']=${dataMap['TEPID']}&trackdataFileName=word.doc");
            </c:if>
            <c:if test="${dataMap['type'] == 'edit'}">
            	document.all.OA1.Open("<%=webapp%>/webword/webwordAction!makeword.dhtml?dataMap['WORDID']=${dataMap['WORDID']}&dataMap['BUSPKEY']=${dataMap['BUSPKEY']}&dataMap['TEPID']=${dataMap['TEPID']}&dataMap['EXTENDOPERATE']=${dataMap.EXTENDOPERATE}&trackdataFileName=word.doc");
            </c:if>
    	}
		function fn_open() {
