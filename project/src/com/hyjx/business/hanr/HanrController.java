package com.hyjx.business.hanr;
...
@Controller
@RequestMapping("/hanr")
public class HanrController {
    @Resource
    private IHanrService service;
    @Resource
    IBaseDao dao;
    ...
    @ResponseBody
    public String queryList(HanrModel model, HttpServletRequest request){
        //分页
        Pager pager = dao.getPager("com.hyjx.business.hanr.queryList","com.hyjx.business.hanr.queryCount",model);
        List<HanrModel> list = pager.getResultList();
        //转码
        for (HanrModel m: list){
            //部门名称，爱好，性别转码
            ...
            if(sclyCode!=null) {
                ...
                for (String str : SclyC) {
                    String sclys = CodeTableManager.getNameByCode("hanr_scly", str);
                    scly.add(sclys);
                }
                String scly1 = "";
            ...
            if(ahCode!=null){
                ...
                for(String str:AhC){
                    String ahs = CodeTableManager.getNameByCode("HANR_AH",str);
                    ah.add(ahs);
                }
                String ah1="";
    ...
    public ModelAndView gotoAdd(HttpServletRequest request){
         ...
         * 爱好，性别码表列
         */
        List<CodeBean> ahList = CodeTableManager.getTableList("HANR_AH");
        List<CodeBean> sex = CodeTableManager.getTableList("sex");
        List<CodeBean> sclyList = CodeTableManager.getTableList("hanr_scly");
        //页面
        ModelAndView mav = new ModelAndView("/business/hanr/addHanr");
    ...
    @ResponseBody
    public String doAdd(HanrModel model){
    ...
    public ModelAndView gotoEdit(HttpServletRequest request) {
        ...
        HanrModel qp = new HanrModel();
        qp.setId(id);
        HanrModel model = (HanrModel)(service.getInfoById(qp));
        ModelAndView mav = new ModelAndView("business/hanr/editHanr");
        mav.addObject("model",model);
        List<CodeBean> ahList = CodeTableManager.getTableList("HANR_AH");
        List<CodeBean> sclyList = CodeTableManager.getTableList("hanr_scly");
        mav.addObject("ahList",ahList);
    ...
    @ResponseBody
    public String doEdit(HanrModel model){
    ...
    public ModelAndView gotoView(HttpServletRequest request) {
        ...
        ModelAndView mav = new ModelAndView("business/hanr/viewHanr");
        mav.addObject("model",model);
        return mav;
    }
